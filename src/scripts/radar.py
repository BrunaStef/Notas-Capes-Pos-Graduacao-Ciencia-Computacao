...
PADRAO_DOCENTES = (
    str(BASE_DIR / "br-capes-colsucup-docente-*.xlsx")
    + ";"
    + str(BASE_DIR / "br-capes-colsucup-docente-*.csv")
)

PADRAO_DISCENTES = (
    str(BASE_DIR / "br-capes-colsucup-discentes-*.xlsx")
    + ";"
    + str(BASE_DIR / "br-capes-colsucup-discentes-*.csv")
)
...
def carregar_base(pattern_string, nome_base):
    ...
    if not arquivos:
        raise RuntimeError(f"Nenhum arquivo encontrado para {nome_base}")
...
def gerar_radar_programa(df_doc, df_disc, global_bounds):
...
def main():
    ...
    concept_year = pd.merge(
        teacher_agg, student_agg, on=["AN_BASE", "CD_CONCEITO_PROGRAMA"], how="outer"
    ...
    menu_opcoes = {}
    mapeamento = df_disc_all.groupby(["SG_ENTIDADE_ENSINO", "CD_PROGRAMA_IES", "NM_PROGRAMA_IES"]).size().reset_index()
    ...
    todos_progs = set(df_disc_all["CD_PROGRAMA_IES"].unique()).union(set(df_doc_all["CD_PROGRAMA_IES"].unique()))
    
    for cd in todos_progs:
        if pd.isna(cd) or cd == "nan": continue
        
        df_d = df_disc_all[df_disc_all["CD_PROGRAMA_IES"] == cd]
        df_dc = df_doc_all[df_doc_all["CD_PROGRAMA_IES"] == cd]
        ...
        json_p = {
            ...
            "radar": gerar_radar_programa(df_dc, df_d, global_bounds),
        ...
        for a in ANOS_DESEJADOS:
            if not df_d[df_d["AN_BASE"] == a].empty: 
                json_p["discentes"]["por_ano"][str(a)] = gerar_metricas_discentes_programa(df_d[df_d["AN_BASE"] == a])
            if not df_dc[df_dc["AN_BASE"] == a].empty: 
                json_p["docentes"]["por_ano"][str(a)] = gerar_metricas_docentes_programa(df_dc[df_dc["AN_BASE"] == a])
